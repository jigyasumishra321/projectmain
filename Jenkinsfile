   ...
   agent any
    stages{
        ...
        stage('Push image to Hub'){
            steps{
                script{
                   withCredentials([string(credentialsId: 'dockerhubpwd', variable: 'dokerlogin')]) {
            ...
            steps{
               dir ('/var/lib/jenkins/workspace/project1') {
                   ...
                   sshagent(['kuberneteslogins']){
                    ...
                    script{
                        try{
                            sh "ssh ubuntu@18.209.4.151 kubectl apply -f deploymentservice.yml"
                        }catch(error){
                            sh "ssh ubuntu@18.209.4.151 kubectl create -f deploymentservice.yml"
                        }
                        }
                    }
